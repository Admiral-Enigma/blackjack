...
;(function() {
  ...
  setInterval(() => {
    for (let i = 0; i < backgroundtoberainbowed.length; i++) {
      backgroundtoberainbowed[i].style.backgroundColor =
        'hsl(' +
        (backgroundcounter + Math.floor(i * backgroundspeed)) +
        ', 100%, 70%'
    }
    backgroundcounter++
  }, 15)
  ...
  for (let id = 0; id < rainbowhover.length; id++) {
    ...
    if (rainbowhover[id].dataset.foxrainbowhover) {
      let hoverelement = document.getElementById(
        rainbowhover[id].dataset.foxrainbowhover
      )
      ...
      hoverelement.addEventListener(
        ...
        function startanimation() {
          hoverinterval[id] = setInterval(() => {
            for (let i = 0; i < rainbowelements[id].length; i++) {
              rainbowelements[id][i].style.color =
                'hsl(' +
                (hovercounters[id][i] + Math.floor(i * hoverspeed)) +
                ', 100%, 70%'
              hovercounters[id][i]++
            }
          }, 7)
      ...
      rainbowhover[id].addEventListener(
        'mouseenter',
        function startanimation() {
          hoverinterval[id] = setInterval(() => {
            for (let i = 0; i < rainbowelements[id].length; i++) {
  ...
  for (let id = 0; id < invertedhover.length; id++) {
    startinterval[id] = setInterval(() => {
      for (let i = 0; i < rainbowinvelements[id].length; i++) {
        rainbowinvelements[id][i].style.color =
          'hsl(' +
          (invcounters[id][i] + Math.floor(i * hoverspeed)) +
          ', 100%, 70%'
        invcounters[id][i]++
      }
    }, 7)
    ...
    if (invertedhover[id].dataset.foxrainbowhover) {
      let hoverelement = document.getElementById(
        invertedhover[id].dataset.foxrainbowhover
      )
      ...
      invertedhover[id].addEventListener(
        'mouseenter',
        function stopanimation() {
          clearInterval(startinterval[id])
          clearInterval(hoverinterval[id])
  ...
  for (let id = 0; id < rainbowhoverbg.length; id++) {
    rainbowhoverbg[id].addEventListener(
      'mouseenter',
      function startbganimation() {
        hoverbginterval[id] = setInterval(() => {
          rainbowhoverbg[id].style.backgroundColor =
            'hsl(' +
            (hoverbgcounter + Math.floor(id * hoverbackgroundspeed)) +
            ', 100%, 70%'
          hoverbgcounter++
        }, 15)
      },
...
function textcolorchange(rainbowarray, rainbowspeed) {
  ...
  setInterval(() => {
    for (let i = 0; i < rainbowarray.length; i++) {
      rainbowarray[i].style.color =
        'hsl(' +
        (counterarray[i] + Math.floor(i * rainbowspeed)) +
        ', 100%, 70%'
